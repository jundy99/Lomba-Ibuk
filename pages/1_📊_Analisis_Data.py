...
try:
    ...
    st.error("File model tidak ditemukan. Jalankan skrip 4 terlebih dahulu.")
    st.stop()

# --- Fungsi Bantuan ---
# Fungsi untuk menghitung BMI dan Status Gizi dari input
...
# --- Logika Prediksi ---
if submit_button:
    # 1. Menghitung fitur-fitur turunan dari input
    ...
    # Menampilkan informasi tambahan untuk pengguna
    ...
    # 2. Membuat DataFrame dari input untuk di-encode
    input_data = {
        'JK_ENCODED': [jk_encoded],
        'USIA_DALAM_BULAN': [usia_bulan],
        'JULI_BB': [bb_juli],
        'JULI_TB': [tb_juli],
        'BMI_JULI': [bmi_juli],
        'TB_PER_USIA': [tb_per_usia],
        'BB_PER_TB': [bb_per_tb],
    ...
    # 4. Menyelaraskan Kolom
    # Memastikan input memiliki kolom yang sama persis dengan yang digunakan saat pelatihan
    # Kolom yang ada saat pelatihan (contoh)
    kolom_pelatihan = ['JK_ENCODED', 'USIA_DALAM_BULAN', 'JULI_BB', 'JULI_TB', 
                       'BMI_JULI', 'TB_PER_USIA', 'BB_PER_TB',
                       'STATUS_GIZI_JULI_Kurus', 'STATUS_GIZI_JULI_Normal', 
                       'STATUS_GIZI_JULI_Obesitas'] # drop_first=True membuat 'Gemuk' jadi basis
